...
export const SeparationFormSchema = TrackSeparationModelInputSchema.extend({
    local_file: z
        .any()
        .refine((files) => files?.length === 1, 'File is required.')
        .refine(
            (files) => files?.[0]?.size <= MAX_LOCAL_FILE_SIZE,
            `Max file size is ${MAX_LOCAL_FILE_SIZE / 1024 / 1024}MB.`,
        )
        .refine(
            (files) =>
                Object.keys(ACCEPTED_LOCAL_FILE_TYPES).includes(
                    files?.[0]?.type,
                ),
            `Supported file types are ${Object.values(ACCEPTED_LOCAL_FILE_TYPES)
