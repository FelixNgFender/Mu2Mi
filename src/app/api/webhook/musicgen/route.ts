...
export const POST = async (req: Request) => {
    ...
    try {
        ...
        const { status, output, error } = body as MusicGenWebhookBody;

        if (error) {
            await trackModel.updateOne(taskId, {
                musicgenStatus: 'failed',
            });
        ...
        if (track.musicgenStatus === 'succeeded' || status === 'starting') {
            return HttpResponse.success();
        }

        if (
            track.musicgenStatus === 'processing' &&
